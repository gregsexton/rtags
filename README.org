...
=rtags-diagnostics= overlays or Flycheck overlays. We recommend setting
=flycheck-highlighting-mode= locally to =nil= as the RTags overlays are more
accurate.
...
with RTags. We trigger it manually cause it will give you a better experience.
To turn of the automatic Flycheck syntax checking, set the variable
